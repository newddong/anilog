...
export default Input24B = props => {
	...
	const getMsg = () => {
		if (confirm == 'normal') {
			return <Text style={(txt.noto22, {color: '#13BE00', lineHeight: 36 * DP, includeFontPadding: false})}></Text>;
		} else if (confirm) {
	...
	return (
		<View style={{flexDirection: 'row'}}>
			...
			<View style={{height: 168 * DP}}>
				...
				<View style={{flexDirection: 'row', height: 40 * DP}}>
					<Text style={[txt.noto24, {color: APRI10, includeFontPadding: false}]}> {props.title} </Text>
					<Text style={[txt.noto22, {color: GRAY20, includeFontPadding: false, marginLeft: 75 * DP}]}> {props.info} </Text>
				</View>
				{/* 하단테두리는 2px, APRI설정 */}
			...
			<View>
				...
				<View style={{height: 80 * DP, backgroundColor: 'blue', width: 10}}></View>
