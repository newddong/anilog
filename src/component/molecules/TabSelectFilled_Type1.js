...
export default TabSelectFilled_Type1 = props => {
	...
	const renderItem = ({item, index}) => {
		return (
			<TouchableOpacity onPress={() => handleClick(index)}>
				<View style={{justifyContent: 'center', alignSelf: 'center', width: 260 * DP, height: 82 * DP, backgroundColor: setBackGroundColor(index)}}>
					<Text style={[txt.noto28, {color: getTextColor(index), textAlign: 'center'}]}>{item}</Text>
				</View>
			</TouchableOpacity>
		);
	};
	return (
		<View>
			<View style={{width: 260 * DP, height: 82 * DP, backgroundColor: 'yellow'}}>
