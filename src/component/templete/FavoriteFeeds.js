			...
			result => {
				...
				if (!selectMode) {
					...
					if (route.name == 'UserFeeds') {
						navigation.push('UserFeedList', {title: titleValue, userobject: feed_id.feed_writer_id, selected: feed_id});
					} else if (route.name == 'TagMeFeeds') {
						// console.log('tageme');
						navigation.push('TagMeFeedList', {title: titleValue + '님을 태그한 글', userobject: feed_id.feed_writer_id, selected: feed_id});
					} else if (route.name == 'FavoriteFeeds') {
