...
export default AnimalProtectRequestDetail = ({route}) => {
	...
	React.useEffect(() => {
		...
		setPhoto(route ? route.params.photo : '');
	}, [route.params.photo]);
				...
				<ScrollView horizontal={true} contentContainerStyle={[animalProtectRequestDetail_style.container]}>
					<View>
						...
						<View style={[temp_style.rescueImage]}>
							<RescueImage
								status={data ? data.protect_request_status : 'adopt'}
								img_uri={data ? data.protect_request_photo_thumbnail : DEFAULT_PROFILE}
							/>
						</View>
