...
export default AssignPetProfileImage = props => {
	...
	React.useEffect(() => {
		props.route.params == null
			...
			: setImgSelected(props.route.params),
			setData({ ...data, user_profile_uri: props.route.params })
	}, [props.route.params]);
	...
	const goToNextStep = () => {
		...
		Modal.popTwoBtn(
			!protect ? '새로 임시보호를 하는 동물이 있습니다. 해당 동물을 등록하시겠습니까?' : '새로 입양하시는 동물이 있습니다. 해당 동물을 등록하시겠습니까?',
			'아니오',
			'등록',
			() => Modal.close(),
			() => {
				Modal.close()
				props.navigation.push('AssignPetInfoA', data)
			}
		)
	}
		...
		<ScrollView>
			...
			<View style={[login_style.wrp_main,]}>
				...
				<View style={[btn_style.btn_w654, assignPetProfileImage_style.btn_w654]}>
					...
					{confirmed
						? <AniButton btnTitle={'확인'} btnTheme={'shadow'} btnStyle={'filled'} btnLayout={btn_w654} titleFontStyle={32} onPress={goToNextStep} />
